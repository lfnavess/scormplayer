	...
	try{
		...
		API = _window.API = window.API = scormPlayer.API = {
				...
				try{
					...
					if(var_ret === "true"){ 
						...
						if(SCO_in_use && SCO_in_use.add(5, "minutes") > moment()){
							...
							var_ret = "false";
							API.LMSFinish("");
						} else if(scormPlayerConfig.testFlash && !Modernizr.flash){
                            console.log(comment("Flash no soportado"));
                            alert("Tu navegador parece que no soporta o esta desactivado el contenido elaborado en Flash.\nComunicaté a Rededucativa para ayudarte.");
							var_ret = "false";
							API.LMSFinish("");
                        } else{
			...
			LMSGetLastError: function (){
				try{
					var start = moment();
					var var_ret = _window._API.LMSGetLastError();
